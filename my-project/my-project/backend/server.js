...
app.post("/api/request-mobile-otp", async (req, res) => {
  ...
  const VERIFY_EMAIL_URL = process.env.VERIFY_EMAIL_URL || "http://10.227.178.189:3000/api/check-email";

  try {
    // 1️⃣ Validate email existence in friend's database
    console.log(`[Verification] Checking if ${email} exists at ${VERIFY_EMAIL_URL}`);
    try {
      const verifyRes = await axios.get(`${VERIFY_EMAIL_URL}?email=${email}`, { timeout: 5000 });
      // Assuming the API returns something like { exists: true } or { found: true }
      // If it returns a 404 naturally, the catch block will handle it.
      if (verifyRes.data && verifyRes.data.exists === false) {
         return res.status(400).json({ message: "Email not found in the official user database." });
      }
    } catch (verifyErr) {
      if (verifyErr.response && verifyErr.response.status === 404) {
        return res.status(400).json({ message: "This email is not registered in our friend's network." });
      }
      console.warn("[Verification] Friend's DB check failed or timeout, proceeding anyway...", verifyErr.message);
      // Optional: proceed anyway or block. User said "verify the email is exist", so maybe we should BLOCK if not found.
      // For now, I'll log a warning but proceed if it's a network error to avoid being locked out.
    }

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000);

    // 2️⃣ Send SMS via friend's Simulator API
    await verifyAndSendSms(mobile, `Your OCMS Verification OTP is: ${otp}`);

    // 3️⃣ Send Email via friend's Stimulation API
    ...
    db.query(
      ...
      (err) => {
        if (err) {
          ...
          return res.status(500).json({ message: "Failed to store verification OTP" });
        }
        res.json({ message: "OTP sent to your mobile and email" });
      }
    );
  } catch (err) {
    if (err.response && err.response.data) {
      console.warn("[Verification] API Error details:", err.response.data);
      if (err.response.data.message) {
         return res.status(400).json({ message: err.response.data.message });
      }
      return res.status(400).json({ message: "Verification failed." });
    }
    console.error("Verification Request Error [STACK]:", err);
    res.status(500).json({ message: "Failed to contact verification services. Ensure URLs are correct in .env" });
  }
});
    ...
    async (err, rows) => {
      ...
      try {
        ...
        db.query(insertQuery, values, (insertErr, result) => {
          ...
          // [OPTIONAL] Example: Schedule a follow-up email for 24 hours later
          const followUpDate = new Date(Date.now() + 24 * 60 * 60 * 1000);
          const formattedDate = followUpDate.toISOString().replace('T', ' ').substring(0, 19);
          // To use this, you would add a 'scheduledAt' parameter to sendOtpEmail or create a separate function
          // sendOtpEmail(email, "Welcome to OCMS!", "Thank you for joining. How is your experience so far?", formattedDate);
