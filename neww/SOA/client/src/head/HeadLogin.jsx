// src/pages/HeadLogin.jsx
import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import "./Headlogin.css"; 


export default function HeadLogin({ onBack }) {
  ...
  const handlePasswordLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/head/login`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        localStorage.setItem("headId", data.headId);
  ...
  const requestLoginOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/head/login/request-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const verifyLoginOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/head/login/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const requestForgotOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/head/forgot/request-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email }),
      });
  ...
  const verifyForgotOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${apiBase}/head/forgot/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="login-container">
        ...
        <main className="login-right">
          <header className="login-header">
            ...
            <h1>Head Login</h1>
            <p>Department head access portal</p>
          </header>
